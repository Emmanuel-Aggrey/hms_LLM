...
MIGRATION_TASK_ARN=$(
  aws ecs run-task \
    --cluster=$cluster \
    --task-definition=mobile_fixit-$cluster-migration \
    --count=1 \
    --launch-type=FARGATE \
...
MIGRATION_LOG=$(
  aws logs get-log-events \
    --start-from-head \
    --log-group-name=/ecs/mobile_fixit-$cluster \
    --log-stream-name=ecs/mobile_fixit-$cluster-migration/$MIGRATION_TASK_ID \
    --output=json |
